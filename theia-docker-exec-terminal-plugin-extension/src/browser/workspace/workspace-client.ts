...
export class WorkspaceClient {
    ...
    public async findRemoteServer(serverName: string): Promise<IServer> {
        const machines = await this.getListMachines();

        for (const machineName in machines) {
            const servers = machines[machineName].servers;
            for (const serverName in servers) {
                if (serverName == remoteServerName) {
                    return servers[remoteServerName];
                }
            }
        }
    ...
    //     return await this.baseEnvVariablesServer.getEnvValueByKey("CHE_API");
    // }
